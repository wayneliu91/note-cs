...
public class ASyncProducer {
    public static void main(String[] args) throws Exception {
        ...
        // 设置 NameServer 地址
        producer.setNamesrvAddr("10.0.1.41:9876;10.0.1.42:9876");
        producer.start();
        // 设置失败重试时间
        ...
        for (int i = 0; i < messageCount; i++) {
            ...
            msg.setTopic("async_topic");
            msg.setTags("tag2");
            msg.setBody(("Hello async:" + i).getBytes(RemotingHelper.DEFAULT_CHARSET));
            // 异步发送，并注册一个回调函数来接收结果
            producer.send(msg, new SendCallback() {
                @Override
                public void onSuccess(SendResult sendResult) {
                    System.out.printf("OK %s %n", sendResult);
                }
...
public class OnewayProducer {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            // 创建消息，并指定Topic，Tag和消息体
            Message msg = new Message();
            msg.setTopic("oneway_topic");
            msg.setTags("tag1");
            msg.setBody(("Hello oneway" + i).getBytes(RemotingHelper.DEFAULT_CHARSET));
            // 发送单向消息，没有任何返回结果
...
public class LoadBalanceConsumer {
    public static void main(String[] args) throws Exception {
        ...
        // 设置 NameServer 地址
        consumer.setNamesrvAddr("10.0.1.41:9876;10.0.1.42:9876");
        // 订阅一个或者多个Topic，以及Tag来过滤需要消费的消息
        consumer.subscribe("sync", "*");

        // 负载均衡模式消费(默认)：多个消费者共同消费队列消息，每个消费者处理的消息不同
        ...
        consumer.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
            for (MessageExt msg : msgs) {
                System.out.printf("%s - %s%n", Thread.currentThread().getName(), new String(msg.getBody()));
            }
            // 标记该消息已经被成功消费
...
public class TransactionProducer {
  public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            try {
                Message msg = new Message();
                msg.setTopic("Transaction_Topic");
